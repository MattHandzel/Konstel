...
class ConversationAgent:
    ...
    async def process_message(
        ...
        if intent.get("action") and intent["action"] != "query":
            modifications = await self._execute_graph_modification(
                intent, constellation
            )

        # Generate response
        response_text = await self._generate_response(
            message, constellation, intent, modifications
        )
...
"""
    ...
    async def _execute_graph_modification(
        self, intent: Dict[str, Any], constellation: ConstellationDetail
    ) -> List[Dict[str, Any]]:
        ...
        if action == "add_node":
            ...
            if matching_node:
                modification = {
                    "type": "modify_node",
                    "data": {"node_id": matching_node.id, "changes": changes},
                }
                modifications.append(modification)
...
"""
        ...
        try:
            ...
            if modifications:
                ...
                return (
                    "I understand your message. How else can I help you with your goal?"
                )
    ...
    def _build_graph_context(self, constellation: ConstellationDetail) -> str:
        ...
        for node in constellation.nodes[:10]:  # Limit to first 10 nodes
            impact_desc = (
                "positive"
                if node.impact_score > 0
                else "negative" if node.impact_score < 0 else "neutral"
            )
            context_parts.append(
                f"- {node.title}: {impact_desc} impact ({node.impact_score:.2f})"
            )

        if len(constellation.nodes) > 10:
            context_parts.append(
                f"... and {len(constellation.nodes) - 10} more factors"
            )

        return "\n".join(context_parts)

    def _find_node_by_title(
        self, constellation: ConstellationDetail, title: str
    ) -> Optional[Any]:
    ...
    def _find_edge_between_nodes(
        self, constellation: ConstellationDetail, source_id: str, target_id: str
    ) -> Optional[Any]:
...
class IntentClassifier:
    ...
    def __init__(self):
        self.intent_patterns = {
            ...
            "remove_node": ["remove", "delete", "eliminate", "take out", "get rid of"],
            "modify_node": ["change", "update", "modify", "edit", "adjust", "alter"],
            "add_edge": ["connect", "link", "relate", "associate", "tie", "influences"],
            "remove_edge": ["disconnect", "unlink", "separate", "break connection"],
            "query": ["what", "how", "why", "which", "show me", "tell me", "explain"],
        }
