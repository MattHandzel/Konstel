...
class RubricItem(BaseModel):
    ...
    suggestion: str = Field(..., min_length=0, max_length=500)


class GoalEvaluation(BaseModel):
    specificity: RubricItem
    measurability: RubricItem
    time_boundedness: RubricItem
    personal_relevance: RubricItem
    achievability: RubricItem
...
class UserProfile(BaseModel):
    ...
    def set(self, key: str, value: Any, data_type: Optional[str] = None) -> None:
        ...
        self.entries[key] = UserProfileEntry(
            key=key, value=str(value), data_type=data_type, updated_at=datetime.now()
        )
